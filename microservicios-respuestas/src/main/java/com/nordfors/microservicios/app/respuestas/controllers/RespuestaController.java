package com.nordfors.microservicios.app.respuestas.controllers;
...
import com.nordfors.microservicios.app.respuestas.services.RespuestaService;
...
public class RespuestaController {
	...
	public ResponseEntity<?> crear(@RequestBody Iterable<Respuesta> respuestas){
		respuestas = ((List<Respuesta>)respuestas).stream().map(r -> {
			// r.getAlumno().getId() alumno que viene en el json(requestbody) en el cuerpo de la peticion
			r.setAlumnoId(r.getAlumno().getId());
			r.setPreguntaId(r.getPregunta().getId());
			// Se retorna cada respuesta emitida pero modificada
			return r;
	...
	public ResponseEntity<?> obtenerExamenesIdsConRespuestasAlumnos(@PathVariable Long alumnoId){
		Iterable<Long> examenesIds = service.findExamenesIdsConRespuestasByAlumnos(alumnoId);
